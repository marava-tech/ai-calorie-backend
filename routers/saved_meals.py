...
async def list_saved_meals(
    ...
    db = get_db()
    docs = await db.saved_meals.find({}).sort("use_count", -1).skip(skip).limit(limit).to_list(None)
...
async def update_saved_meal(
    ...
    result = await db.saved_meals.update_one({"_id": oid}, {"$set": update_data})
...
async def delete_saved_meal(meal_id: str, _: str = Depends(get_current_user)):
    db = get_db()
    oid = parse_object_id(meal_id, "meal_id")
    result = await db.saved_meals.delete_one({"_id": oid})
...
async def use_saved_meal(meal_id: str, meal_slot: str, _: str = Depends(get_current_user)):
    ...
    db = get_db()
    oid = parse_object_id(meal_id, "meal_id")
    meal = await db.saved_meals.find_one({"_id": oid})
    if not meal:
        raise HTTPException(404, "Saved meal not found")
